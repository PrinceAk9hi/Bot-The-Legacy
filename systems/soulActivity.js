const { Events, EmbedBuilder, ActionRowBuilder, StringSelectMenuBuilder, ButtonBuilder, ButtonStyle, MessageFlags } = require('discord.js');
...
function register(client) {
    ...
    function payload(period = 'all', page = 0, owner = null) {
        ...
        for (const [index, r] of rows.slice(page*10,page*10+10).entries()) embed.addFields({name:`#${page*10+index+1}`,value:`<@${r.id}> — 💬 **${r.messages.toLocaleString('fr-FR')}** messages\n🎙️ **${duration(r.voiceMs)}**`});
        if (!rows.length) embed.addFields({name:'Membres',value:'Aucun membre disponible pour le moment.'});
        const components = [new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId(KEY).setPlaceholder('Choisir une période').addOptions(Object.entries(PERIODS).map(([value,[label]])=>({label,value,default:value===period}))))];
        if (owner) components.push(new ActionRowBuilder().addComponents(
    ...
    client.once(Events.ClientReady, async () => {
        ...
        try { publicMessage = await ensurePanel(client, CHANNEL, KEY, payload()); } catch { console.warn('⚠️ Panel activité indisponible : vérifier les permissions du salon.'); }
        save();
    });
    ...
    client.on(Events.InteractionCreate, async i => {
        ...
        try {
            if(i.isStringSelectMenu() && i.customId===KEY){ await i.deferReply({flags:MessageFlags.Ephemeral});return i.editReply(payload(i.values[0],0,i.user.id)); }
