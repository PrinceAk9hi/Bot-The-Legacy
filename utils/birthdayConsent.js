...
function payload(id,date,p={},inGuild=false){
 ...
 .setDescription(`Tu as renseigné ta date de naissance dans ton profil =bienvenue${birthDate(date)?' : **'+date.slice(0,5)+'**':''}. Merci de choisir séparément :\n\n**1. Partage public** : autorises-tu l’affichage de ton jour et de ton mois de naissance dans le classement des anniversaires ? **Ton année de naissance et ton âge ne seront pas publiés.**\n\n**2. Rôle anniversaire** : souhaites-tu recevoir le rôle **Joyeux anniversaire** le jour J ? Il sera retiré le lendemain, heure de Paris.`)
 .addFields({name:'Partage dans le classement',value:answer(p.sharePublic)},{name:'Rôle Joyeux anniversaire',value:answer(p.enabled)})
...
async function handle(i){if(!i.isButton()||!i.customId.startsWith('birthdayconsent:'))return;
 ...
 await i.editReply(payload(id,profile.birthDate,read('birthdayPreferences')[id],Boolean(i.guildId)));
 if(i.client?.birthdayBoard)await i.client.birthdayBoard.refresh().catch(e=>console.error('Actualisation calendrier :',e.code||e.message));
}
